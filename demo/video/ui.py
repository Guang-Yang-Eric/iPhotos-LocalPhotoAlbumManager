"""Qt widgets — ThumbnailBar, VideoEditor and supporting controls."""
...
class HandleButton(QPushButton):
    """Custom handle button that draws a thick white arrow."""
...
class ThumbnailBar(QWidget):
    """Thumbnail strip that draws all pixmaps directly via QPainter."""
    ...
    def __init__(self, parent=None):
        ...
        # Thumbnail canvas (custom painted)
    ...
    def set_strip(self, pixmap):
        """Set a single pre-composed strip pixmap (contact-sheet path).

        No per-thumbnail scaling — the strip is already at the target size.
        """
    ...
    def add_thumbnail(self, pixmap):
        ...
        Scales to physical pixel height for crisp HiDPI rendering when
        HIDPI_MODE == 'hidpi'; otherwise uses pixmap as-is.
        """
        if HIDPI_MODE == "hidpi":
            dpr = self.devicePixelRatioF()
...
class _ThumbnailCanvas(QWidget):
    """Lightweight widget that paints pixmaps side by side."""
...
class VideoEditor(QMainWindow):
    ...
    def get_video_info(self, video_path):
        """Use ffprobe to get video info (API compat)."""
    ...
    def generate_thumbnails(self, video_path):
        ...
        # In normal mode, generate at logical pixel height (faster).
        # In hidpi mode, generate at physical pixel height (crisper).
        dpr = self.devicePixelRatioF()
        if HIDPI_MODE == "hidpi":
            target_height = int(THUMB_LOGICAL_HEIGHT * dpr)
        else:
            target_height = THUMB_LOGICAL_HEIGHT
    ...
    def _on_single_thumbnail(self, result):
        """Slot: handle a single thumbnail or a complete strip."""
        kind = result[0]

        if kind == 'strip':
            # Contact-sheet path: one pre-composed horizontal strip.
            _, w, h, buf = result
            img = QImage(
            ...
            if not pix.isNull():
                self.thumb_strip.set_strip(pix)
            return
