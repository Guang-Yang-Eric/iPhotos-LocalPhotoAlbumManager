# 删除与移动媒体操作性能优化方案（后续规划）

> **范围:** pybind11 / C++ 加速层 (未来需求)  
> **说明:** 纯 Python 内部优化已完成并归档到 `docs/finished/requirements/MOVE_DELETE_OPTIMIZATION_PLAN_PYTHON.md`
...
### 2.1 `iphoto_native.file_ops` — 批量文件操作
    ...
    for (const auto& src_str : sources) {
        ...
        try {
            ...
            try {
                ...
                fs::copy(src, target, fs::copy_options::none);
                fs::remove(src);
                r.target = target.string();
                r.success = true;
...
### 2.2 `iphoto_native.metadata` — 内嵌元数据提取
...
**建议：** 仅在纯 Python 优化无法满足需求时推进 C++ 加速层。
