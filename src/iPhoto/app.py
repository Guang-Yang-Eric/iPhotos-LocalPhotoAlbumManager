...
from .application.services.index_sync_service import IndexSyncService
from .application.services.path_normalizer import PathNormalizer
...
def _compute_album_path(root: Path, library_root: Optional[Path]) -> Optional[str]:
    """Delegate to :class:`PathNormalizer`."""
    return PathNormalizer.compute_album_path(root, library_root)
...
def _ensure_links(
    ...
    """Delegate to :class:`IndexSyncService`."""
    IndexSyncService.ensure_links(root, rows, library_root=library_root)


def _compute_links_payload(rows: List[dict]) -> tuple[List[LiveGroup], Dict[str, object]]:
    """Delegate to :class:`IndexSyncService`."""
    return IndexSyncService.compute_links_payload(rows)
...
def _sync_live_roles_to_db(
    ...
    """Delegate to :class:`IndexSyncService`."""
    IndexSyncService.sync_live_roles_to_db(root, groups, library_root=library_root)


def _normalise_rel_key(rel_value: object) -> Optional[str]:
    """Delegate to :class:`PathNormalizer`."""
    return PathNormalizer.normalise_rel_key(rel_value)
...
def _update_index_snapshot(
    ...
    """Delegate to :class:`IndexSyncService`."""
    IndexSyncService.update_index_snapshot(root, materialised_rows, library_root=library_root)
