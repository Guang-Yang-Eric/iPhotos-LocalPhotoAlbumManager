"""Service encapsulating asset-restoration logic extracted from AppFacade."""
...
class RestorationService:
    """Restore previously deleted assets to their original locations."""
    ...
    def __init__(
        ...
        *,
        ...
        restore_prompt_handler_getter: Callable[[], Optional[Callable[[str], bool]]],
        error_callback: Callable[[str], None],
    ) -> None:
        self._library_manager_getter = library_manager_getter
        self._model_provider = model_provider
        self._move_service = move_service
        self._restore_prompt_handler_getter = restore_prompt_handler_getter
    ...
    def restore_assets(self, sources: Iterable[Path]) -> bool:
        ...
        if trash_root is None:
            self._error_callback("Recently Deleted folder is unavailable.")
            return False
        ...
        for raw_path in sources:
            ...
            if not candidate.exists():
                self._error_callback(f"File not found: {candidate}")
                continue
            try:
    ...
    def _determine_restore_destination(
        ...
        *,
        ...
        def _offer_restore_to_root(
            ...
            prompt = self._restore_prompt_handler_getter()
            if prompt is None:
                self._error_callback(skip_reason)
                return None
            if prompt(filename):
                return library_root
            self._error_callback(decline_reason)
            return None
