...
from ....utils.ffmpeg import apply_video_rotation, extract_oriented_video_frame

# Backward-compatible alias for existing unit tests.
_apply_pil_rotation = apply_video_rotation
...
def grab_video_frame(
    ...
    *,
    ...
    """Return a thumbnail frame oriented like the detail-page player."""

    target_size = (max(size.width(), 1), max(size.height(), 1))

    for target in _seek_targets(still_image_time, duration):
        pil_image = extract_oriented_video_frame(path, at=target, scale=target_size)
