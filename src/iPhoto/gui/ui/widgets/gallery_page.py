...
class GalleryPageWidget(QWidget):
    ...
    def set_cluster_gallery_mode(self, enabled: bool) -> None:
        ...
        # On Linux some compositors do not propagate the visibility change
        # immediately.  Explicitly invalidating the layout and requesting a
        # repaint ensures the header disappears (or appears) without waiting
        # for the next spontaneous update.
        lay = self.layout()
        if lay is not None:
            lay.invalidate()
        self.update()
